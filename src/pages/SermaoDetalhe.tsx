...
const SermaoDetalhe: React.FC = () => {
  const { slug } = useParams<{ slug: string }>();
  const navigate = useNavigate();
  const [sermao, setSermao] = useState<SermaoDetalhes | null>(null);
  ...
  useEffect(() => {
    ...
    const query = `*[_type == "sermao" && slug.current == $slug][0]{
      _id,
      titulo,
      "pregador": pregador->nome,
      outroPregador,
      dataPregacao,
      referenciaBiblica,
      conteudoEscrito,
    ...
    console.log('SermaoDetalhe: Buscando sermão com slug:', slug);
    setLoading(true);
    setError(null);

    sanityClient.fetch<SermaoDetalhes>(query, { slug })
      .then((data) => {
        console.log('SermaoDetalhe: Sermão recebido:', data);
        if (!data) {
          setError('Sermão não encontrado');
        } else {
          setSermao(data);
        }
        setLoading(false);
      ...
      .catch((err) => {
        console.error('SermaoDetalhe: Erro ao buscar sermão:', err);
        setError('Falha ao carregar o sermão.');
        setLoading(false);
      });
  }, [slug]);
      ...
      image: ({value}: any) => {
        ...
        return (
          <figure className="my-8">
            <img
              src={imageUrl}
              alt={value.alt || 'Imagem do sermão'}
    ...
    <div className="container mx-auto px-4 py-16 min-h-screen">
      ...
      {loading && (
        <div className="flex justify-center items-center py-10">
          ...
          <p className="ml-3 text-gray-600 dark:text-gray-400">Carregando sermão...</p>
        </div>
      )}
      ...
      {error && (
        <div className="text-center py-10 px-4 bg-red-100 dark:bg-red-900 border border-red-400 dark:border-red-700 text-red-700 dark:text-red-200 rounded">
          ...
          <Link to="/artigos" className="mt-4 inline-block text-blue-600 dark:text-blue-400 hover:underline">
            Voltar para a lista de sermões
          </Link>
        </div>
      )}

      {!loading && !error && sermao && (
        <article className="bg-white dark:bg-gray-800 rounded-lg shadow-lg overflow-hidden max-w-4xl mx-auto border-l-4 border-blue-600 dark:border-blue-500">
          {/* Cabeçalho do Sermão */}
            ...
            <div className="flex items-center justify-between mb-2">
              ...
              {sermao.dataPregacao && (
                <div className="flex items-center text-sm text-gray-600 dark:text-gray-400">
                  ...
                  <time dateTime={sermao.dataPregacao}>{formatData(sermao.dataPregacao)}</time>
                </div>
              )}
            </div>
            <h1 className="text-3xl font-bold text-gray-900 dark:text-white mb-4">{sermao.titulo || 'Sermão Sem Título'}</h1>
            <div className="flex flex-wrap items-center gap-4">
              {(sermao.pregador || sermao.outroPregador) && (
                <div className="flex items-center text-sm text-gray-600 dark:text-gray-400">
                  <User size={16} className="mr-1" />
                  <span>Por {sermao.pregador || sermao.outroPregador}</span>
                </div>
              )}
              {sermao.referenciaBiblica && (
                <div className="text-sm text-gray-600 dark:text-gray-400 font-medium">
                  Referência: {sermao.referenciaBiblica}
                </div>
              )}
            </div>
          </header>
          ...
          {sermao.imagemDestaqueUrl && (
            <div className="w-full">
              <img 
                src={sermao.imagemDestaqueUrl} 
                alt={sermao.titulo || 'Imagem do sermão'} 
          ...
          <div className="p-6 prose prose-lg dark:prose-invert max-w-none">
            {sermao.conteudoEscrito ? (
              <PortableText 
                value={sermao.conteudoEscrito} 
                components={portableTextComponents} 
              />
            ) : (
              <p className="text-gray-500 dark:text-gray-400 italic">Este sermão não possui conteúdo escrito.</p>
            )}
          </div>
